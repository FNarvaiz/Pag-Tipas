...
function trySetRSFieldValue(fieldName, data)
  ...
  next
  if b then
    response.write("OK - " & fieldName & " = " & data & "<br>")
  else
    response.write("Error al cargar campo " & fieldName & " = " & data & "<br>")
  end if
end function
...
function updateBinaryData
  ...
  if len(objUpload("uploadFormFileField").fileName) > 0 then
    ...
    trySetRSFieldValue dbFieldBaseName & "_CONTENTTYPE", objUpload("uploadFormFileField").contentType
    ...
    trySetRSFieldValue dbFieldBaseName & "_CONTENTTYPE", null
    trySetRSFieldValue dbFieldBaseName & "_BINARYDATA", null
  end if
