...
var server = "front-end/forms/forms.asp";
var gridRowIdSeparator = "@";
var firstFormRecordId = -1;
var appBaseWindowTitle = null;
...
function loadFormContainer(formContainerId, containerId)
{
  ...
  ajaxGetText(server + "?sessionId=" + sessionId + "&verb=formContainer&formId=" + formContainerId, containerId, loadingSignalId,
    function() {
      ...
      e.scrollLeft = 0;
      e.scrollTop = 0;
      if (appBaseWindowTitle) document.title = document.getElementById("formContainerTitleText").innerHTML + " - " + appBaseWindowTitle;
...
function htmlEditorDialog(formId, fieldName)
{
  if (loadingForms) return;
  htmlContentFieldFormId = formId;
  htmlContentFieldName = fieldName;
  htmlFieldElem = document.forms[formId].elements[htmlContentFieldName + "NewValue"];
  try
  {
    var editor = FCKeditorAPI.GetInstance('FCKeditor1');
    editor.SetData(htmlFieldElem.value, true);
    document.getElementById("htmlEditorTitle").innerHTML = document.getElementById(formId + fieldName + "Label").innerHTML;
    showDialog("htmlEditorDialog");
  }
  catch (e)
  {
  	ajaxGetText(buildRequestUrl("htmlEditorDialog", formId) + "&dbFieldBaseName=" + fieldName, "dialogsPanel", loadingSignalId,
  	  function ()	{
